...
public class HelpModule extends BaseModule {
    ...
    public void update(Minecraft client) {
        ...
        // Pie Graph (Shift+F3)
        lines.get(0).setValue(client.options.renderDebugCharts ? Utils.getStyledText(visible,
                TextColor.fromLegacyFormat(ChatFormatting.GREEN))
                :  Utils.getStyledText(hidden, TextColor.fromLegacyFormat(ChatFormatting.RED)));

        // FPS / TPS (Alt+F3)
        lines.get(1).setValue(client.options.renderFpsChart ? Utils.getStyledText(visible,
                TextColor.fromLegacyFormat(ChatFormatting.GREEN))
                :  Utils.getStyledText(hidden, TextColor.fromLegacyFormat(ChatFormatting.RED)));
